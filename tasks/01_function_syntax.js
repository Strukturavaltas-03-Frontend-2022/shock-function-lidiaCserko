...
const getProductDescrition = product => {
    //
    const titleCaseName = product.name[0].toUpperCase() + product.name.slice(1);
    const sellPrice = Math.round(product.price * 1.35);
    return `${titleCaseName}, $${sellPrice}, in stock: ${product.stock}`;
}
...
// törölni
const getBio = member => {
    const titleCaseName = member.name[0].toUpperCase() + member.name.slice(1);
    return `${titleCaseName}, ${member.age} years old, live in: ${member.city}.`;
}
// törölni
...
const getMemberDesc = (member = {name: 'jimmy', birthYear: 2000, city: 'SF'}) => {
    ...
    const titleCaseName = member.name[0].toUpperCase() + member.name.slice(1);
    const age = new Date().getFullYear() - member.birthYear;
    return `${titleCaseName}, ${age} years old, live in: ${member.city}.`;
}
...
const getCarDetails = (car = {manufacturer: 'ford', year: 2010, type: 'fiesta'}) => {
    const titleCaseMan = car.manufacturer[0].toUpperCase() + car.manufacturer.slice(1);
    const titleCaseType = car.type[0].toUpperCase() + car.type.slice(1);
    const age = new Date().getFullYear() - car.year;
    return `${titleCaseMan}, ${age} years old, type: ${titleCaseType}.`;
}
// törölni
